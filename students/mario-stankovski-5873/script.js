...
async function loadEpisodes() {
    try {
        ...
        if (CONFIG.USE_MULTI_LOAD) {
            ...
            console.log('🚀 Using Alternative 2: Loading from multiple URLs for +5 bonus points');
        ...
        if (Array.isArray(data)) {
            ...
            throw new Error('Invalid data format: expected array or object with episodes');
        }
...
function displayEpisodes(episodes) {
    ...
    episodes.forEach((episode, index) => {
        ...
        row.innerHTML = `
            ...
            <td data-era="${episode.era || 'Unknown'}">${episode.era || 'Unknown'}</td>
...
function validateData(episodes) {
    ...
    episodes.forEach((episode, index) => {
        ...
        if (episode.series < 0) {
            console.warn(`Episode ${index + 1}: Negative series number (${episode.series})`);
            warningCount++;
        }
        ...
        if (broadcastYear > currentYear) {
            console.warn(`Episode ${index + 1}: Future broadcast date (${episode.broadcast_year})`);
            warningCount++;
        }
    });
...
async function loadFromMultipleUrls() {
    console.log('📦 Loading episodes from', CONFIG.MULTI_DATA_URLS.length, 'different endpoints...');
    
    const promises = CONFIG.MULTI_DATA_URLS.map(async (url, index) => {
        try {
            console.log(`🔄 Fetching batch ${index + 1}/${CONFIG.MULTI_DATA_URLS.length}: ${url}`);
            ...
            if (!response.ok) {
                throw new Error(`HTTP ${response.status}: ${response.statusText} for URL ${index + 1}`);
            }
            
            const batchData = await response.json();
            console.log(`✅ Batch ${index + 1} loaded:`, Array.isArray(batchData) ? batchData.length : 'object', 'episodes');
    ...
    console.log('🎉 Multi-URL loading complete! Total episodes loaded:', allEpisodes.length);
    console.log('📊 Episodes per batch:', batches.map(batch => batch.length));
...
function populateFilters() {
    ...
    // Get unique values
    const eras = [...new Set(state.episodes.map(ep => ep.era).filter(Boolean))].sort();
    const doctors = [...new Set(state.episodes.map(ep => formatDoctor(ep.doctor)).filter(Boolean))].sort();
    const companions = [...new Set(state.episodes.map(ep => formatCompanion(ep.companion)).filter(c => c !== 'None'))].sort();
    ...
    doctors.forEach(doctor => {
        doctorSelect.innerHTML += `<option value="${escapeHtml(doctor)}">${escapeHtml(doctor)}</option>`;
    });
    ...
    companions.forEach(companion => {
        companionSelect.innerHTML += `<option value="${escapeHtml(companion)}">${escapeHtml(companion)}</option>`;
    });
    
    console.log('🎛️ Filters populated:', {eras: eras.length, doctors: doctors.length, companions: companions.length});
}
...
function exportToCSV() {
    try {
        const episodes = state.filtered.length > 0 ? state.filtered : state.episodes;
        ...
        link.download = `doctor-who-episodes-${new Date().toISOString().split('T')[0]}.csv`;
...
function updateHeroStats() {
    ...
    const uniqueDoctors = [...new Set(state.episodes.map(ep => formatDoctor(ep.doctor)))].length;
